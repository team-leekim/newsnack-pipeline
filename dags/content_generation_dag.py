...
with DAG(
    ...
    generate_top5 = SimpleHttpOperator(
        ...
        method='POST',
        data="{{ task_instance.xcom_pull(task_ids='select_target_issues', key='top_5_issues') | tojson }}",
    ...
    generate_extra = SimpleHttpOperator(
        ...
        endpoint='/ai-articles',
        method='POST',
        data="{{ task_instance.xcom_pull(task_ids='select_target_issues', key='extra_issues') | tojson }}",
        headers={"Content-Type": "application/json"},
        response_check=lambda response: response.status_code in [200, 201],
        log_response=True,
    )
